...
@php
    ...
    $allowedCount = $selectedRole?->permissions?->where('allowed', true)->count() ?? 0;
    ...
    <a href="{{ route('admin.system.roles', ['role_id' => $role->id]) }}" style="text-decoration:none;color:inherit;">
        <div class="card card-pad" style="border-left:3px solid {{ $role->color }};{{ $selectedRole && $selectedRole->id === $role->id ? 'outline:2px solid var(--brand-1);' : '' }}">
            <div class="flex items-center justify-between" style="gap:12px;">
                <div><b style="font-size:14px;">{{ $role->name }}</b><div class="cell-sub">{{ $role->slug }}</div></div>
...
<div class="grid-3" style="align-items:start;">
    <div style="grid-column:span 2;">
        @if (auth()->user()->canAccessModule('Roles & Permissions', 'Edit'))
        <div class="card card-pad" style="margin-bottom:16px;">
        ...
        <div class="card">
            <div class="card-head">
                <div><h3>Permission Matrix — {{ $selectedRole->name }}</h3><div class="card-head__sub">View controls section visibility. Add/Edit/Delete/Publish/Export control protected actions.</div></div>
            </div>
                            ...
                            <tr>
                                ...
                                @foreach($actions as $action)
                                    @php
                                        ...
                                        $on = $supported && $selectedRole->can($module, $action);
                                    @endphp
                                    <td style="text-align:center;{{ !$supported ? 'opacity:.28;' : '' }}">
                                        @if ($supported)
                                            <label style="cursor:pointer;display:inline-block;" title="{{ $module }} → {{ $action }}">
                                                <input type="checkbox" data-action="{{ $action }}" name="permissions[]" value="{{ $module }}|{{ $action }}" class="switch-checkbox" {{ $on ? 'checked' : '' }} {{ auth()->user()->canAccessModule('Roles & Permissions', 'Edit') ? '' : 'disabled' }}>
    ...
    <div>
        ...
        <div class="card card-pad" style="margin-bottom:16px;">
            ...
            <div class="cell-sub" style="line-height:1.65;margin-top:8px;">A user must first have <b>Administrator</b> access. Their permission role then limits what they can see and do. An administrator with no granular role remains a <b>Full Access Administrator</b> for backward compatibility.</div>
        </div>

        @if (auth()->user()->canAccessModule('Roles & Permissions', 'Delete'))
