...
<tr>
<td><div class="dr-record"><span><i data-lucide="{{ $source->type==='rss'?'rss':'braces' }}"></i></span><div><strong>{{ $source->name }}</strong><small>{{ $source->default_category ?: 'No default category' }}</small><form method="POST" action="{{ route('admin.system.news-sources.authority',$source->id) }}" style="display:flex;gap:6px;margin-top:7px;align-items:center">@csrf<select class="select" name="authority_type" style="min-width:115px">@foreach(['official','research','major_publication','specialist','publication','aggregator'] as $at)<option value="{{ $at }}" @selected($source->authority_type===$at)>{{ str_replace('_',' ',ucwords($at,'_')) }}</option>@endforeach</select><input class="input" name="authority_score" type="number" min="0" max="100" value="{{ $source->authority_score }}" style="width:72px"><button class="btn btn-secondary btn-sm" type="submit">Save</button></form></div></div></td>
