...
<x-page-header title="AI Companies" subtitle="Manage AI companies and their connected tools and models" :breadcrumb="['AI Management','AI Companies']">
    <x-slot:actions>@if(auth()->user()->canAccessModule('AI Companies','Add'))<a href="{{ route('admin.companies.create') }}" class="btn btn-primary btn-sm"><i data-lucide="plus"></i> Add Company</a>@endif</x-slot:actions>
</x-page-header>
...
<td><div class="flex gap-8">@if(auth()->user()->canAccessModule('AI Companies','Edit'))<a class="icon-btn" href="{{ route('admin.companies.edit',$company->id) }}" title="Edit"><i data-lucide="pencil"></i></a>@endif @if(auth()->user()->canAccessModule('AI Companies','Delete'))<form method="POST" action="{{ route('admin.companies.destroy',$company->id) }}" onsubmit="return confirm('Delete {{ addslashes($company->name) }}? Related tools/models will be kept but detached.')">@csrf @method('DELETE')<button class="icon-btn" type="submit" title="Delete"><i data-lucide="trash-2"></i></button></form>@endif</div></td>
