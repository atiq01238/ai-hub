...
<div class="benchmark-list">@foreach($model->benchmarkResults as $result)<div><span><strong>@if($result->benchmark?->is_active && $result->benchmark?->slug)<a class="model-benchmark-link" href="{{ route('benchmarks.show',$result->benchmark) }}">{{ $result->benchmark->name }}<i data-lucide="arrow-up-right"></i></a>@else{{ $result->benchmark?->name ?? 'Benchmark' }}@endif</strong><small>{{ $result->benchmark?->benchmark_class_label ?? 'Unclassified' }} · {{ $result->source_name ?: 'Verified result' }}{{ $result->tested_at ? ' · '.$result->tested_at->format('M Y') : '' }}</small></span><div><i style="width:{{ min(100,(float)$result->score) }}%"></i></div><b>{{ number_format((float)$result->score,1) }}</b></div>@endforeach</div>
...
<section id="guides" class="detail-block model-guides-block">
    <div class="block-title-row"><div><span class="section-kicker">GUIDES & ANALYSIS</span><h2>Research related to {{ $model->name }}</h2></div><a href="{{ route('articles.index') }}">All AI guides <i data-lucide="arrow-right"></i></a></div>
    <p class="block-intro">Published analysis is shown only when it connects through this model, its provider or its product category.</p>
    <div class="model-guide-grid">
        @foreach($relatedArticles as $article)
        <a href="{{ route('articles.show',$article) }}" class="model-guide-card"><span><small>{{ $article->category ?: 'AI Guide' }}</small>@if($article->published_at)<em>{{ $article->published_at->format('M j, Y') }}</em>@endif</span><strong>{{ $article->title }}</strong><p>{{ Str::limit($article->summary,115) }}</p><b>Read analysis <i data-lucide="arrow-right"></i></b></a>
        @endforeach
    </div>
</section>
@endif
...
@if($relatedModels->isNotEmpty())<section id="related" class="detail-block"><div class="block-title-row"><div><span class="section-kicker">KEEP EXPLORING</span><h2>Related AI models</h2></div><a href="{{ route('models.index') }}">View all models <i data-lucide="arrow-right"></i></a></div><div class="related-models">@foreach($relatedModels as $related)<a href="{{ route('models.show',$related) }}"><img src="{{ $related->logo_url }}" alt=""><span><strong>{{ $related->name }}</strong><small>{{ $related->company?->name }} · {{ $related->context_window ?: 'Context N/A' }}</small></span><b>{{ $related->benchmark_score !== null ? number_format((float)$related->benchmark_score,1) : '—' }}</b></a>@endforeach</div></section>@endif</main>
...
@if($latestNews->isNotEmpty())<div class="side-card"><span class="side-label">LATEST RELEVANT UPDATES</span><div class="side-news">@foreach($latestNews as $news)<a href="{{ route('news.show',$news) }}"><article>@if($news->image_path)<img src="{{ $news->image_url }}" alt="{{ $news->headline }}">@endif<div><strong>{{ Str::limit($news->headline,68) }}</strong><small>{{ $news->published_at?->diffForHumans() }}</small></div></article></a>@endforeach</div></div>@endif</aside></div></section>
@endsection
