...
<section id="test-lab" class="detail-block model-lab-section">
    ...
    <div class="model-lab-summary">
        ...
        <div><span>Verified results</span><strong>{{ $labStats['verified'] }}</strong><small>Evidence reviewed</small></div>
    </div>
    @if($labStats['types']->isNotEmpty())
    <div class="model-lab-types">@foreach($labStats['types'] as $type)<div><span>{{ $type['label'] }}</span><strong>{{ number_format((float)$type['score'],1) }}</strong><small>{{ $type['tests'] }} {{ Str::plural('test',$type['tests']) }}</small></div>@endforeach</div>
    @endif
    <div class="model-lab-results">
        @foreach($labResults as $labResult)
        <a href="{{ route('testlab.show',$labResult->test) }}">
            <span><strong>{{ $labResult->test?->name }}</strong><small>{{ $labResult->test?->testTypeLabel() }} · {{ $labResult->run_count }} {{ Str::plural('run',$labResult->run_count) }} · {{ $labResult->verificationLabel() }}</small></span>
            <span class="model-lab-score"><b>{{ number_format((float)$labResult->overall_score,1) }}</b>@if($labResult->score_min !== null && $labResult->score_max !== null)<small>{{ number_format((float)$labResult->score_min,1) }}–{{ number_format((float)$labResult->score_max,1) }}</small>@endif</span>
