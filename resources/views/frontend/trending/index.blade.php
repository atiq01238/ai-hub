    ...
    <section class="intel-section">
        ...
        <div class="trend-news-grid">
            @foreach($news->take($tab==='news'?10:6) as $rank=>$item)
                <article class="trend-news-card"><a class="trend-news-image" href="{{ route('news.show',$item) }}"><img src="{{ $item->image_url ?: '/images/frontend/content-placeholder.svg' }}" alt="{{ $item->headline }}" onerror="this.style.display='none'"><span>{{ $item->category ?: 'AI News' }}</span></a><div><small>#{{ $rank+1 }} · {{ $item->company?->name ?? $item->source ?? 'AI Hub' }} · {{ optional($item->published_at)->diffForHumans() }}</small><h3><a href="{{ route('news.show',$item) }}">{{ $item->headline }}</a></h3><p>{{ \Illuminate\Support\Str::limit($item->summary ?: $item->ai_summary,125) }}</p><footer><span><i data-lucide="signal-high"></i> Importance {{ (int)$item->importance }}/100</span>@if($item->verification_status==='verified')<span class="verified-pill"><i data-lucide="badge-check"></i>Verified</span>@endif</footer></div></article>
            @endforeach
        </div>
    </section>
    @endif
