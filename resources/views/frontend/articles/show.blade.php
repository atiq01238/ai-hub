...
<section class="article-reading"><div class="content-wrap article-reading-grid"><article class="article-body"><div class="article-summary-box"><span>THE BRIEF</span><p>{{ $article->summary ?: 'A practical AI Orbit analysis focused on the information that matters for product and model decisions.' }}</p></div><div class="article-prose">{!! \App\Support\ArticleContent::render($article->content ?: $article->summary) !!}</div>@if($tags->isNotEmpty())<div class="article-tags"><span>Topics</span>@foreach($tags as $tag)<a href="{{ route('articles.index',['q'=>$tag]) }}">#{{ $tag }}</a>@endforeach</div>@endif
@if($relatedTools->isNotEmpty() || $relatedModels->isNotEmpty())<section class="article-related-block"><div class="block-heading"><span>MENTIONED IN THIS ARTICLE</span><h2>Related AI products</h2></div><div class="article-product-grid">@foreach($relatedTools as $tool)@php $logo=$tool->logo_url; @endphp<a href="{{ route('tools.show',$tool) }}"><img src="{{ $logo }}" alt=""><div><small>AI TOOL</small><strong>{{ $tool->name }}</strong><span>{{ $tool->company?->name }}</span></div><i data-lucide="arrow-up-right"></i></a>@endforeach @foreach($relatedModels as $model)@php $logo=$model->logo_url; @endphp<a href="{{ route('models.show',$model) }}"><img src="{{ $logo }}" alt=""><div><small>AI MODEL</small><strong>{{ $model->name }}</strong><span>{{ $model->company?->name }}</span></div><i data-lucide="arrow-up-right"></i></a>@endforeach</div></section>@endif
...
@if($relatedArticles->isNotEmpty())<div class="content-wrap related-reading"><div class="block-heading"><span>KEEP READING</span><h2>Related intelligence</h2></div><div class="related-reading-grid">@foreach($relatedArticles as $related)<a href="{{ route('articles.show',$related) }}"><small>{{ $related->category ?: 'Article' }}</small><h3>{{ $related->title }}</h3><p>{{ \Illuminate\Support\Str::limit($related->summary,100) }}</p><span>{{ optional($related->published_at)->format('M j, Y') }} <i data-lucide="arrow-right"></i></span></a>@endforeach</div></div>@endif
</section>
@endsection
