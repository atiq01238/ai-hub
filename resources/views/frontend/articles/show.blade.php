...
@php
    ...
    $articleSchema = [
        ...
        'publishingPrinciples' => route('editorial-guidelines'),
        'mainEntityOfPage' => [
            '@' . 'type' => 'WebPage',
...
$outline=collect($articleOutline ?? [])->where('level',2)->values();
$showUpdated=filled($article->updated_at) && filled($article->published_at) && $article->updated_at->gt($article->published_at->copy()->addDay());
$lastEditorialReview = $article->approved_at ?: null;
@endphp
<section class="article-detail-hero"><div class="content-wrap"><div class="content-breadcrumb"><a href="{{ route('home') }}">Home</a><i data-lucide="chevron-right"></i><a href="{{ route('articles.index') }}">Articles</a><i data-lucide="chevron-right"></i><span>{{ \Illuminate\Support\Str::limit($article->title,45) }}</span></div><div class="article-detail-grid"><div><span class="article-category">{{ $article->category ?: $article->categoryTerm?->name ?: 'Article' }}</span><h1>{{ $article->title }}</h1><p class="article-deck">{{ $article->summary }}</p><div class="article-byline"><span><i data-lucide="user-round"></i>{{ $article->author?->name ?? 'AI Orbit Editorial' }}</span><span><i data-lucide="calendar-days"></i>{{ optional($article->published_at)->format('F j, Y') }}</span><span><i data-lucide="clock-3"></i>{{ $readTime }} min read</span>@if($showUpdated)<span><i data-lucide="refresh-cw"></i>Updated {{ $article->updated_at->format('M j, Y') }}</span>@endif @if($article->reviewer)<span><i data-lucide="badge-check"></i>Reviewed by {{ $article->reviewer->name }}</span>@endif @if($lastEditorialReview)<span><i data-lucide="clipboard-check"></i>Editorial review {{ $lastEditorialReview->format('M j, Y') }}</span>@endif</div></div><div class="article-detail-image"><img src="{{ $image }}" alt="{{ $article->title }}"><span><i data-lucide="shield-check"></i>{{ $article->approval_status === 'approved' ? 'Editorially approved' : 'Published article' }}</span></div></div></div></section>
<section class="article-reading"><div class="content-wrap article-reading-grid"><article class="article-body"><div class="article-summary-box"><span>THE BRIEF</span><p>{{ $article->summary ?: 'A practical AI Orbit analysis focused on the information that matters for product and model decisions.' }}</p></div><div class="article-trust-note"><i data-lucide="shield-check"></i><div><strong>AI Orbit editorial standard</strong><p>Editorial analysis is kept separate from provider claims and measured results. Automated tools may assist organization or drafting, while factual claims remain subject to sourcing and correction controls.</p><span><a href="{{ route('editorial-guidelines') }}">Editorial guidelines</a><b>•</b><a href="{{ route('sourcing-verification') }}">Sourcing & verification</a><b>•</b><a href="{{ route('corrections-policy') }}">Corrections</a></span></div></div>@if($outline->count() >= 3)<nav class="article-toc" aria-label="Article contents"><div><span>IN THIS GUIDE</span><strong>Quick navigation</strong></div><ol>@foreach($outline as $item)<li><a href="#{{ $item['id'] }}">{{ $item['text'] }}</a></li>@endforeach</ol></nav>@endif<div class="article-prose">{!! \App\Support\ArticleContent::render($article->content ?: $article->summary) !!}</div>
...
<aside class="article-aside"><div class="side-card article-author"><span class="side-label">PUBLISHED BY</span><div class="author-avatar">{{ strtoupper(substr($article->author?->name ?? 'AI',0,2)) }}</div><h3>{{ $article->author?->name ?? 'AI Orbit Editorial' }}</h3><p>AI Orbit editorial content is structured around product intelligence, benchmarks and practical decision-making.</p></div><div class="side-card article-trust-card"><span class="side-label">EDITORIAL TRANSPARENCY</span><div class="article-trust-row"><span>Approval</span><strong>{{ $article->approval_status === 'approved' ? 'Editorially approved' : Str::headline($article->approval_status ?: 'Published') }}</strong></div>@if($article->reviewer)<div class="article-trust-row"><span>Reviewer</span><strong>{{ $article->reviewer->name }}</strong></div>@endif @if($lastEditorialReview)<div class="article-trust-row"><span>Review date</span><strong>{{ $lastEditorialReview->format('M j, Y') }}</strong></div>@endif<div class="article-trust-row"><span>Last updated</span><strong>{{ $article->updated_at?->format('M j, Y') ?: '—' }}</strong></div><a class="side-action" href="{{ route('editorial-guidelines') }}">How we publish <i data-lucide="arrow-right"></i></a><a class="side-action" href="{{ route('corrections-policy') }}">Report a correction <i data-lucide="arrow-right"></i></a></div>@if($article->company)<div class="side-card"><span class="side-label">COMPANY</span><h3>{{ $article->company->name }}</h3><p>{{ \Illuminate\Support\Str::limit($article->company->description,110) }}</p><a class="side-action" href="{{ route('companies.show',$article->company) }}">Company profile <i data-lucide="arrow-right"></i></a></div>@endif<div class="side-card"><span class="side-label">SHARE</span><div class="share-row"><button type="button" data-save-item data-save-type="article" data-save-id="{{ $article->id }}" aria-label="Save article" aria-pressed="false"><i data-lucide="bookmark"></i></button><button type="button" onclick="navigator.clipboard?.writeText(location.href)"><i data-lucide="link"></i></button><a href="mailto:?subject={{ rawurlencode($article->title) }}&body={{ rawurlencode(request()->fullUrl()) }}"><i data-lucide="mail"></i></a></div></div></aside></div>
