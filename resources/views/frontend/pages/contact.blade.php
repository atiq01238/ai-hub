    ...
    <section class="inst-contact-form panel">
        ...
        <form action="{{ route('contact.store') }}" method="POST" class="inst-form">@csrf
            ...
            <div class="inst-form-grid">
                ...
                @php $contactTopic = old('topic', request('topic', 'general')); @endphp
                <label><span>Topic <b>*</b></span><select name="topic" required><option value="general" @selected($contactTopic==='general')>General question</option><option value="feedback" @selected($contactTopic==='feedback')>Product feedback</option><option value="data_correction" @selected($contactTopic==='data_correction')>Data quality / correction</option><option value="partnership" @selected($contactTopic==='partnership')>Partnership</option><option value="press" @selected($contactTopic==='press')>Press / media</option><option value="technical" @selected($contactTopic==='technical')>Technical issue</option></select></label>
                <label><span>Subject <b>*</b></span><input name="subject" maxlength="180" required value="{{ old('subject', request('subject')) }}" placeholder="Short summary"></label>
