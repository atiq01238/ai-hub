...
use App\Models\{AiModel,Article,Category,Company,NewsItem,SavedSearch,SearchEvent,Tool};
use Illuminate\Http\Request;
use Illuminate\Support\Str;
class SearchController extends Controller {
 public function index(Request $request){
  $query=trim((string)$request->query('q','')); $type=(string)$request->query('type','all');
  $allowed=['all','tools','models','news','companies','articles']; if(!in_array($type,$allowed,true))$type='all';
  $counts=array_fill_keys(array_slice($allowed,1),0); $tools=$models=$news=$companies=$articles=collect();
  if($query!==''){
   ...
   $qs=['tools'=>$toolQ,'models'=>$modelQ,'news'=>$newsQ,'companies'=>$companyQ,'articles'=>$articleQ];
  ...
  return view('frontend.search.index',compact('query','type','counts','tools','models','news','companies','articles','popularCategories','trendingTools','recentSearches','savedSearches')+['total'=>array_sum($counts)]);
 }
