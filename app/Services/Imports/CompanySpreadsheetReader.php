...
class CompanySpreadsheetReader
{
    ...
    private function normalizeRows(array $rows): array
    {
        ...
        if (count($rows) < 2) {
            throw new RuntimeException('The spreadsheet does not contain any company rows.');
        }
        ...
        if (! isset($map['name'])) {
            throw new RuntimeException('A "Company name" or "Name" column is required.');
        }
        ...
        foreach (array_slice($rows, 1) as $rowNumber => $row) {
            ...
            $result[] = [
                ...
                'name' => $name,
                'website' => $this->cell($row, $map['website'] ?? null),
                'founded_year' => $this->cell($row, $map['founded_year'] ?? null),
                'status' => $this->cell($row, $map['status'] ?? null) ?: 'active',
                'description' => $this->cell($row, $map['description'] ?? null),
                'source_url' => $this->cell($row, $map['source_url'] ?? null),
                'category' => $this->cell($row, $map['category'] ?? null),
            ];
        }
    ...
    private function headerKey(string $header): ?string
    {
        return match (trim($header)) {
            ...
            'website', 'website url', 'url' => 'website',
            'founded year', 'founded', 'year founded' => 'founded_year',
            'operational status', 'status' => 'status',
            'company overview', 'overview', 'description', 'company description' => 'description',
